...
const ACTION_CONFIG = {
  STRONG_BUY: { label: 'STRONG BUY', color: 'text-emerald-400', bg: 'bg-emerald-400/10 border-emerald-400/30', icon: TrendingUp },
  BUY: { label: 'BUY', color: 'text-up', bg: 'bg-up/10 border-up/20', icon: TrendingUp },
  HOLD: { label: 'HOLD', color: 'text-yellow-400', bg: 'bg-yellow-400/10 border-yellow-400/20', icon: Minus },
  SELL: { label: 'SELL', color: 'text-down', bg: 'bg-down/10 border-down/20', icon: TrendingDown },
  STRONG_SELL: { label: 'STRONG SELL', color: 'text-red-400', bg: 'bg-red-400/10 border-red-400/30', icon: TrendingDown },
}

const RISK_CONFIG = {
  LOW: { icon: Shield, color: 'text-up', label: 'Low Risk' },
  MEDIUM: { icon: AlertTriangle, color: 'text-yellow-400', label: 'Medium Risk' },
  HIGH: { icon: Zap, color: 'text-down', label: 'High Risk' },
}
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="ml-auto text-right">
          <div className="text-xs text-gray-500 mb-0.5">Upside</div>
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs mb-1.5">
          <span className="text-gray-400 flex items-center gap-1"><Cpu size={10} /> AI Confidence</span>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <ScoreBar label="Sentiment" score={rec.scores.sentiment} color={scoreColor(rec.scores.sentiment)} />
        <ScoreBar label="Technical" score={rec.scores.technical} color={scoreColor(rec.scores.technical)} />
        <ScoreBar label="Momentum" score={rec.scores.momentum} color={scoreColor(rec.scores.momentum)} />
        <ScoreBar label="Analyst" score={rec.scores.analyst} color={scoreColor(rec.scores.analyst)} />
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5 text-xs">
        ...
        <span className="text-gray-500">{rec.newsCount} related articles</span>
      </div>
    </div>
  )
}
