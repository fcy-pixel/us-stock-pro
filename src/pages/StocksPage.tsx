...
export default function StocksPage() {
      ...
      <div className="space-y-6 animate-slide-up">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'P/E Ratio', value: quote.pe > 0 ? quote.pe.toFixed(1) : 'N/A' },
            { label: 'EPS', value: quote.eps > 0 ? formatPrice(quote.eps) : 'N/A' },
  ...
  // Screener list
  const sectors = ['All', ...Array.from(new Set(stockData.map(q => q.sector)))]
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Filter and analyze {stockData.length} US stocks</p>
      </div>
      ...
      <div className="hidden lg:grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 px-4 text-xs text-gray-500 font-medium uppercase tracking-wider">
        ...
        {(['price', 'changePercent', 'volume', 'marketCap'] as SortKey[]).map(k => (
          <button key={k} onClick={() => toggleSort(k)} className="flex items-center gap-1 hover:text-white transition-colors">
            {k === 'changePercent' ? 'Change %' : k === 'marketCap' ? 'Mkt Cap' : k.charAt(0).toUpperCase() + k.slice(1)}
